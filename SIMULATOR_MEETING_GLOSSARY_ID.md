...
| `Policy status` | Status rekomendasi akhir dari skenario. | Biasanya `candidate`, `risky`, atau `rejected`. |
...
| `Rejected settings` | Bagian konfigurasi atau kondisi yang dianggap bermasalah. | Menjelaskan kenapa skenario tidak ideal atau perlu revisi. |
...
| Status | Arti meeting version |
| --- | --- |
| `candidate` | Skenario masih berada dalam batas yang aman dan layak dibahas untuk pilot. |
| `risky` | Skenario belum gagal total, tapi ada warning yang perlu perhatian founder/tim. |
| `rejected` | Skenario melanggar batas keselamatan utama dan sebaiknya tidak dipakai sebagai default pilot. |
...
| `Completed Run Comparison` | Tabel perbandingan metric antar run. | Lihat per metric, lalu bandingkan tiap skenario di baris yang sama. |
| `Run Statuses` | Tabel ringkas run, scenario, snapshot, status, dan waktu selesai. | Dipakai untuk memastikan run yang dibandingkan memang completed dan konteksnya jelas. |
